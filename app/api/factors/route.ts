...
export async function GET(req: NextRequest) {
  ...
  try {
    const total = await prisma.factor.count({ where: filter });
    const list = await prisma.factor.findMany({
      where: filter,
      skip: range[0],
    ...
    res.headers.set("Content-Range", `factors ${range[0]}-${range[1]}/${total}`);
    return res;
  } catch (e) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const created = await prisma.factor.create({ data });
